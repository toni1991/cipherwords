...
from gameInitializer import CardClasses
...
def flip(x, y):
	...
	return redirect(url_for('getExplain')) # Return explain, as only explain can flip the cards!
...
def choose(x, y):
	...
	return redirect(url_for('getGuess')) # Return guess, as only guess view can choose the cards!
...
def calculateIdentifier():
	current_app.currentGame['identifier'] = GameInitializer.getGameJwt(current_app.currentGame['game'])
...
def setCardsCount():
	...
	def cardsCountInner(dataName, color):
		data[dataName] = 0
	...
	cardsCountInner("blueCards", CardClasses.BLUE)
	cardsCountInner("redCards", CardClasses.RED)
